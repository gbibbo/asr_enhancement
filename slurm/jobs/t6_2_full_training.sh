#!/usr/bin/env bash
# T6.2 full-training Slurm job — runs `_run_training` against
# configs/training/full_training.yaml on a Surrey a100 node for the
# 20 000-step schedule defined by the config. Closes T6.2 only when:
#   * Slurm State=COMPLETED with ExitCode 0:0;
#   * verify JSON validation_passed=true;
#   * the six-artifact contract is finalized;
#   * checkpoint metadata + state-dict roundtrip pass.
# TIMEOUT does NOT close T6.2 under the current trainer (no graceful
# finalization on timeout in scope here).
#
...
#
# Submission (manual, only after the GPU preflight passes):
#   ./slurm/tools/on_submit.sh sbatch /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t6_2_full_training.sh

#SBATCH --job-name=asr_t6_2_full_training
#SBATCH --output=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t6_2_full_training_%j.out
#SBATCH --error=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t6_2_full_training_%j.err
...
RUN_DIR="$TRAIN_ROOT/runs/t6_2_full_training_${JOBID}"
ARTIFACT_DIR="$TRAIN_ROOT/artifacts"
VERIFY_JSON="$ARTIFACT_DIR/t6_2_full_training_verify_${JOBID}.json"
...
echo "=== T6.2 full-training start: $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
echo "Host:                 $(hostname)"
...
# ----------------------------------------------------------------------------
# Step 1: full-training run. ASR_EXPECT_CUDA=1 is the hard guard.
# ----------------------------------------------------------------------------
echo "--- Step 1: full-training run ---"
apptainer exec --nv \
  --env PYTHONNOUSERSITE=1 \
  --env PYTHONPATH="${REPO}:${PREFIX}" \
  --env XDG_CACHE_HOME="$CACHE_ROOT" \
  ...
  python3 -s "$REPO/scripts/training/train_enhancer.py" \
    ...
    --run-id "t6_2_full_training_${JOBID}" \
    --force-fresh
...
# 8. Pass/fail decision. T6.2 closure requires CUDA actually used.
...
echo "=== T6.2 full-training end: $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
exit "$verify_rc"
