#!/usr/bin/env bash
#SBATCH --job-name=asr_t4_2d_whisper_enhanced_smoke
...
# T4.2d smoke run: 5 utterance_ids x 5 families = 25 enhanced records, base.en, CPU only.
#
# Submit from datamove1 via the repo wrapper (NOT inside this script):
#   ./slurm/tools/on_submit.sh sbatch \
#     /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t4_2d_whisper_enhanced_smoke.sh
#
...
# Smoke must NOT pass --summary-md; the committed report is produced only at Gate D.
...
RUN_DIR="$TRAIN_ROOT/runs/t4_2d_whisper_enhanced_smoke_${SLURM_JOB_ID:-local}"
...
echo "=== T4.2d Whisper enhanced eval (smoke) ==="
echo "Host:                     $(hostname)"
...
echo ""
echo "=== T4.2d smoke step complete ==="
echo "Run dir: $RUN_DIR"
