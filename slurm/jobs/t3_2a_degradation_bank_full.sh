#!/usr/bin/env bash
#SBATCH --job-name=asr_t3_2a_bank_full
#SBATCH --output=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t3_2a_degradation_bank_full_%j.out
#SBATCH --error=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t3_2a_degradation_bank_full_%j.err
#SBATCH --time=01:30:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=12G

# T3.2a full run: 2693 records x 5 families = 13465 degraded WAV files.
# Submit only AFTER the smoke gate passes, and only via:
#   ./slurm/tools/on_submit.sh sbatch \
#     /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t3_2a_degradation_bank_full.sh
# Surrey constraints: --bind disabled, --pwd disabled. CPU-only.
...
RUN_DIR="$TRAIN_ROOT/runs/t3_2a_bank_full_${JID}"
STAGING_AUDIO_ROOT="$TRAIN_ROOT/datasets/degraded_build/degradation_v1_full_${JID}"
STAGING_MANIFEST_TMP="$TRAIN_ROOT/datasets/librispeech_manifest_v1_filtered_degraded_v1_${JID}.jsonl.tmp"
FINAL_AUDIO_ROOT="$TRAIN_ROOT/datasets/degraded/degradation_v1"
FINAL_MANIFEST="$TRAIN_ROOT/datasets/librispeech_manifest_v1_filtered_degraded_v1.jsonl"
...
echo "=== T3.2a degradation bank (full) ==="
echo "Host:          $(hostname)"
...
apptainer exec \
  ...
  python3 -s "$REPO/scripts/training/build_degradation_bank.py" \
    ...
    --summary-md              "$REPO/reports/training/degradation_bank_v1.md"

echo ""
echo "=== T3.2a full step complete ==="
echo "Run dir: $RUN_DIR"
