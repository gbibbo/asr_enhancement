#!/usr/bin/env bash
#SBATCH --job-name=asr_t3_1_smoke
#SBATCH --output=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t3_1_whisper_baseline_smoke_%j.out
#SBATCH --error=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t3_1_whisper_baseline_smoke_%j.err
#SBATCH --time=00:45:00
#SBATCH --cpus-per-task=4
#SBATCH --mem=8G

# T3.1 smoke run: 50 records, base.en, CPU only.
#
# Submit from datamove1 via the repo wrapper:
#   ./slurm/tools/on_submit.sh sbatch \
#     /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t3_1_whisper_baseline_smoke.sh
#
...
RUN_DIR="$TRAIN_ROOT/runs/t3_1_baseline_smoke_${SLURM_JOB_ID:-local}"
...
apptainer exec \
  ...
  python3 -s "$REPO/scripts/training/run_whisper_baseline.py" \
    ...
    --out-dir       "$RUN_DIR" \
    --max-records   50 \
    --summary-md    "$REPO/reports/training/baseline_clean_wer_smoke.md"

echo ""
echo "=== T3.1 smoke step complete ==="
echo "Run dir: $RUN_DIR"
