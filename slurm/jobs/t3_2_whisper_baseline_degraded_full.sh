...
#     /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t3_2_whisper_baseline_degraded_full.sh
#
...
RUN_DIR="$TRAIN_ROOT/runs/t3_2_baseline_degraded_full_${SLURM_JOB_ID:-local}"
...
echo "=== T3.2 Whisper degraded baseline (full) ==="
echo "Host:          $(hostname)"
...
apptainer exec \
  ...
  python3 -s "$REPO/scripts/training/run_whisper_baseline_degraded.py" \
    ...
    --out-dir                 "$RUN_DIR" \
    --summary-md              "$REPO/reports/training/baseline_degraded_wer.md"

echo ""
echo "=== T3.2 full step complete ==="
echo "Run dir: $RUN_DIR"
