#!/usr/bin/env bash
# T6.3b — full post-hoc Whisper evaluation Slurm job.
...
# 533 records per family (2 665 transcriptions total) with Whisper on
# CUDA. Produces the final non-placeholder per-family WER/WA artifacts
# the T6.3 summary report consumes. Submit only after T6.3a smoke passes.
#
...
#
# Submission (manual, only after T6.3a smoke passes):
#   ./slurm/tools/on_submit.sh sbatch /mnt/fast/nobackup/users/gb0048/asr_enhancement/slurm/jobs/t6_3_post_hoc_whisper_full.sh

#SBATCH --job-name=asr_t6_3_post_hoc_whisper_full
#SBATCH --output=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t6_3_post_hoc_whisper_full_%j.out
#SBATCH --error=/mnt/fast/nobackup/scratch4weeks/gb0048/asr_enhancement_training/logs/asr_t6_3_post_hoc_whisper_full_%j.err
...
EVAL_OUT_DIR="$TRAIN_ROOT/runs/t6_3_post_hoc_whisper_full_${JOBID}"
ARTIFACT_DIR="$TRAIN_ROOT/artifacts"
VERIFY_JSON="$ARTIFACT_DIR/t6_3_post_hoc_whisper_full_verify_${JOBID}.json"
...
# Capture git state before entering Apptainer.
...
echo "=== T6.3b post-hoc Whisper full eval start: $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
echo "Host:                 $(hostname)"
...
# ----------------------------------------------------------------------------
# Step 1: post-hoc Whisper full evaluation.
# ----------------------------------------------------------------------------
echo "--- Step 1: post-hoc Whisper full evaluation ---"
apptainer exec --nv \
  --env PYTHONNOUSERSITE=1 \
  --env PYTHONPATH="${REPO}:${PREFIX}" \
  --env XDG_CACHE_HOME="$CACHE_ROOT" \
  ...
  python3 -s "$REPO/scripts/training/train_enhancer.py" \
    ...
    --whisper-device cuda \
    --run-id "t6_3_post_hoc_whisper_full_${JOBID}"
...
result = {
    ...
    "errors": [],
    "phase": "t6_3_post_hoc_whisper_full",
...
echo "=== T6.3b post-hoc Whisper full eval end: $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
exit "$verify_rc"
