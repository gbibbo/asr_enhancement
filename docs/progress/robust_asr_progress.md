...
- **Protocol fix recorded:** `APPROVE_PLAN(P9.1)` packet now sits in `state_transport.latest_approval_packet` (scope=task, task_id=P9.1, decision=APPROVE_PLAN, accepted_report_commit=`655a05cfa0f13aad97bb0133b94dfee06bd39684`, next_expected_task=P9.1). The prior `APPROVE_EXECUTION(P9.0)` packet on `f7a195b8…` has been demoted to `prior_approval_packet_p9_0_exec`. **Administrative protocol fix only.** No package, no tag, no evidence, no claims, no markers, and no task state changed. P9.1 still awaits orchestrator `APPROVE_EXECUTION(P9.1)`.
