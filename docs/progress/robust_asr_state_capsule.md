...
Updated by: P1.3 PARTIAL — public LibriSpeech manifests built; OOD-real / demo-reserved splits skipped with `SKIPPED_OOD_PUBLIC_DEFERRED`. `tasks.P1.3.status=PARTIAL`, `tasks.P1.3.marker=BLOCKED_OOD_PUBLIC`, `tasks.P1.3.next_task=P1.4`. `current_task` advanced P1.3 -> P1.4; `last_completed_task` advanced P1.2 -> P1.3. Manifests: librispeech_lora_train (22507 rows, 200 spk, 79.585 h, sha256 7896175e…), librispeech_router_train (6032, 51, 21.006 h, c3d281ab…), librispeech_validation (2703, 40, 5.388 h, 977a6f01…), librispeech_locked_test (2620, 40, 5.404 h, ad4f401e…). Total 33862 rows, 111.382 h. Verifications: OK_PUBLIC_MANIFESTS, OK_MANIFEST_SUMMARY, OK_REPORT_SHAPE, OK_SPEAKER_DISJOINT (non-trivial); 61/61 P0.4 + P1.2 pytest non-regression PASS in 1.42s. No Slurm, no Apptainer, no GPU, no external API; host Python 3.9.13, wall-clock 10.151s for 33862 .flac files. `state_transport.last_accepted_report_commit` STAYS b049f94 (per orchestrator instruction; not advanced to the P1.3 commit). `state_transport.expected_next_task=P1.4`. `latest_approval_packet`=APPROVE_PLAN(P1.3) on 7579602 (next P1.4); `prior_approval_packet`=APPROVE_EXECUTION(P1.3-scope-change) on 7579602 (next P1.3). `markers=[BLOCKED_OOD_PUBLIC]` held; `blocked=false` held; `claims_enabled.ood_real=false` held. P1.4 not started.
Date: 2026-05-08
