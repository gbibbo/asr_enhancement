...
T6.2c — *CPU micro-validation training via Slurm*. T6.2c is a
**subgate inside T6.2**, not the closure of T6.2. It runs the new
T6.2b code path (`spectral_unet_small_v1` + paired dataset +
composite loss + checkpoint helpers) on a small subset of the
`devclean_speaker_split_v1` split for a small number of steps inside
the project Apptainer image on a CPU compute node, and emits the
six-artifact run contract. T6.2c does **not** run full training, does
**not** run Whisper, and does **not** close T6.2. Cut T3 remains
active. T6.2 closure (the actual full training run per plan §16)
remains a later gate after T6.2c. T6.2c must not start unless the
