#!/usr/bin/env python3
"""
B15 cached-example integrity validator.

Reads the declarative B15-05 smoke-results deliverable and asserts that
every multi_network_smoke_result_record observed all curated cached
examples resolving. A record reporting
cached_example_observed=curated_example_failed is a cached-example
...
Emits OK_B15_CACHED_EXAMPLE_OK on PASS or
...
SENTINEL_PASS = "OK_B15_CACHED_EXAMPLE_OK"
SENTINEL_FAIL = "B15_CACHED_EXAMPLE_REGRESSION"

RECORD_KEY = "multi_network_smoke_result_record"
FIELD = "cached_example_observed"
EXPECTED = "all_curated_examples_resolved"
LEGAL_VALUES = {"all_curated_examples_resolved", "curated_example_failed"}
...
def main() -> int:
    ...
    lines = [
        "# B15 Cached-Example Integrity Validation",
        "",
    ...
    for idx, r in enumerate(records, start=1):
        ...
        if value not in LEGAL_VALUES:
            ...
            failures.append(
                f"{rid}: {FIELD} is '{value}'; every curated cached example "
                f"must be observed as '{EXPECTED}'"
            )
    ...
    lines += [
        ...
        "## Result",
        "",
        "Every curated cached example resolved across every result record.",
        "",
        SENTINEL_PASS,
    ]
    out_path.write_text("\n".join(lines) + "\n")
