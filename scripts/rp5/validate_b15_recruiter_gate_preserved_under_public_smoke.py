#!/usr/bin/env python3
"""
B15 recruiter-gate-preserved-under-public-smoke validator.

Reads the declarative B15-05 smoke-results deliverable and asserts that
every multi_network_smoke_result_record observed the recruiter HTTPBasic
application-layer gate as the sole authority for public access:
recruiter_gate_observed_status must be authenticated_access_only. Any
record reporting unauthenticated_access_observed is a recruiter-gate
...
Emits OK_B15_RECRUITER_GATE_PRESERVED on PASS or
...
SENTINEL_PASS = "OK_B15_RECRUITER_GATE_PRESERVED"
...
EXPECTED = "authenticated_access_only"
LEGAL_VALUES = {"authenticated_access_only", "unauthenticated_access_observed"}
...
def main() -> int:
    ...
    lines = [
        "# B15 Recruiter-Gate-Preserved-Under-Public-Smoke Validation",
        "",
    ...
    for idx, r in enumerate(records, start=1):
        ...
        if value not in LEGAL_VALUES:
            ...
            failures.append(
                f"{rid}: {FIELD} is '{value}'; the recruiter gate must be "
    ...
    lines += [
        ...
        "## Result",
        "",
        "The recruiter HTTPBasic gate was preserved under public smoke "
        "across every result record.",
        "",
        SENTINEL_PASS,
    ]
    out_path.write_text("\n".join(lines) + "\n")
