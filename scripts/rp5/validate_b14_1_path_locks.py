#!/usr/bin/env python3
"""
B14.1 path lock closure validator.

Checks that all files in the given git diff fall within the approved B14.1
path locks declared in docs/plans/b14_1/agent_plan.md section 1, plus the
...
def _is_b14_1_test_path(p):
    name = pathlib.Path(p).name
    if p.startswith("tests/demo/") and name.startswith("test_b14_1_") and name.endswith(".py"):
        return True
...
def _is_b14_1_script_path(p):
    name = pathlib.Path(p).name
    if p.startswith("scripts/rp5/") and name.startswith("validate_b14_1_") and name.endswith(".py"):
        return True
    if p.startswith("scripts/rp5/fixtures/") and name.startswith("generate_fixture_validate_b14_1_") and name.endswith(".py"):
        return True
    if p.startswith("scripts/rp5/") and name.startswith("smoke_b14_1_") and name.endswith(".py"):
        return True
    return False


def _is_b14_1_report_path(p):
    name = pathlib.Path(p).name
    return p.startswith("reports/rp5/") and name.startswith("b14_1_") and name.endswith(".md")


def _is_b14_1_config_path(p):
    if p == ".env.example":
        return True
...
def _is_b14_1_api_demo_path(p):
    if not p.startswith("services/api/app/"):
        return False
    name = pathlib.Path(p).name
    # Application-layer exposure-flag plumbing and OpenAPI/docs visibility
    # wiring live under services/api/app/. The recruiter middleware (B14.0)
    # may be inspected; B14.1 task-level diffs may amend only the bounded
    # change-set capped by max_files=6 in agent_plan §1.
...
def _is_b14_1_frontend_path(p):
    if not p.startswith("services/frontend/app/demo/"):
        return False
    return True


def _is_b14_1_tunnel_template_path(p):
    if not p.startswith("infra/tunnel/"):
        return False
    name = pathlib.Path(p).name
    # Per agent_plan §1 PL-B14_1-TUNNEL-TEMPLATE: placeholder template file
    # only, max_files=1. The template carries no real Tailscale auth-key,
    # no Cloudflare token, no stable hostname literal, no public URL literal;
    # those checks are owned by validate_b14_1_no_tunnel_secret_leak and
    # validate_b14_1_no_public_url_or_hostname_literal, not by this path lock.
...
PATH_LOCKS = [
    {"lock_id": "PL-B14_1-API-DEMO", "pattern": _is_b14_1_api_demo_path, "max_files": 6},
    {"lock_id": "PL-B14_1-FRONTEND", "pattern": _is_b14_1_frontend_path, "max_files": 4},
    {"lock_id": "PL-B14_1-CONFIG", "pattern": _is_b14_1_config_path, "max_files": 2},
    {"lock_id": "PL-B14_1-TESTS", "pattern": _is_b14_1_test_path, "max_files": 12},
    {"lock_id": "PL-B14_1-SCRIPTS", "pattern": _is_b14_1_script_path, "max_files": 18},
    {"lock_id": "PL-B14_1-REPORTS", "pattern": _is_b14_1_report_path, "max_files": 20},
    {"lock_id": "PL-B14_1-TUNNEL-TEMPLATE", "pattern": _is_b14_1_tunnel_template_path, "max_files": 1},
...
def main():
    ...
    lines += [
        ...
        "## Result",
        "",
        "All changed files are within approved B14.1 path locks.",
        "",
