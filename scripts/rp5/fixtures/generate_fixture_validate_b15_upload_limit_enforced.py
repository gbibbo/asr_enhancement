#!/usr/bin/env python3
"""
Fixture generator paired with validate_b15_upload_limit_enforced.
...
Negative cases exercised:
  * a record reporting upload_limit_observed=not_enforced;
  * a record reporting an illegal upload_limit_observed value.

The generator performs no public network call and starts no public
exposure.

Emits OK_FIXTURE_VALIDATE_B15_UPLOAD_LIMIT_ENFORCED on PASS or
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B15_UPLOAD_LIMIT_ENFORCED"
SENTINEL_FAIL = "B15_UPLOAD_LIMIT_REGRESSION"
VALIDATOR_OK = "OK_B15_UPLOAD_LIMIT_ENFORCED"
KINDS_ALLOWED = ["positive_and_negative"]

VALIDATOR_NAME = "validate_b15_upload_limit_enforced"
...
def negative_bodies() -> dict[str, str]:
    not_enforced = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    not_enforced[0][FIELD] = "not_enforced"
    illegal = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    illegal[1][FIELD] = "partially_enforced"
    return {
        "negative_upload_limit_not_enforced": render_results(not_enforced),
        "negative_illegal_upload_value": render_results(illegal),
    }
