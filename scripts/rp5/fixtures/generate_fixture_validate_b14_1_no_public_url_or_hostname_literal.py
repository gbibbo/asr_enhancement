#!/usr/bin/env python3
"""
Fixture generator paired with validate_b14_1_no_public_url_or_hostname_literal.

Materializes positive and negative content samples that exercise the
two forbidden classes: public URLs and stable tunnel hostnames. The
generator runs the validator in-process against a temporary --root that
contains the fixture itself, and asserts that each negative case is
flagged with the expected marker while the positive case passes.

Hostname-shaped material in the negative fixtures uses a synthetic
public-suffix style label (placeholder.synthetic.example.invalid-tunnel)
intentionally distinct from any real operator hostname. The validator
matches the broader tunnel-host pattern and would also flag any real
public hostname; this generator never embeds operator-owned literals.

Emits OK_FIXTURE_VALIDATE_B14_1_NO_PUBLIC_URL_OR_HOSTNAME_LITERAL on PASS
or B14_1_PUBLIC_URL_LITERAL_COMMITTED on FAIL.
"""
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B14_1_NO_PUBLIC_URL_OR_HOSTNAME_LITERAL"
SENTINEL_FAIL = "B14_1_PUBLIC_URL_LITERAL_COMMITTED"
...
POSITIVE_BODY = (
    ...
    "  https://[::1]/demo/health\n"
    "No public hostnames, no public URLs.\n"
)


def negative_bodies() -> dict[str, tuple[str, str]]:
    # Each entry: case_name -> (file body, expected marker).
    # Synthetic placeholder host labels: assembled at runtime so the
    # generator source itself contains no literal public host string.
...
def main() -> int:
    ...
    fixtures_root = (
        ...
        / "generate_fixture_validate_b14_1_no_public_url_or_hostname_literal"
    )
    ...
    if pos_sentinel != "OK_B14_1_NO_PUBLIC_URL_OR_HOSTNAME_LITERAL":
        adversarial_failures.append(
            f"positive: expected OK_B14_1_NO_PUBLIC_URL_OR_HOSTNAME_LITERAL "
            f"got {pos_sentinel!r}"
        )

    positive_entry = {
        "case_name": "positive",
        "root": "positive",
        "expected_sentinel": "OK_B14_1_NO_PUBLIC_URL_OR_HOSTNAME_LITERAL",
        "observed_sentinel": pos_sentinel,
        "tree_files": hash_tree(pos_root),
    }

    manifest = {
        "validator": "validate_b14_1_no_public_url_or_hostname_literal",
        "kind": args.kind,
        "owned_markers": [SENTINEL_FAIL, MARKER_HOSTNAME],
