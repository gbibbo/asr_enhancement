#!/usr/bin/env python3
"""
Fixture generator paired with validate_b15_cached_example_integrity.
...
Negative cases exercised:
  * a record reporting cached_example_observed=curated_example_failed;
  * a record reporting an illegal cached_example_observed value.

The generator performs no public network call and starts no public
exposure.

Emits OK_FIXTURE_VALIDATE_B15_CACHED_EXAMPLE_INTEGRITY on PASS or
B15_CACHED_EXAMPLE_REGRESSION on FAIL.
"""
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B15_CACHED_EXAMPLE_INTEGRITY"
SENTINEL_FAIL = "B15_CACHED_EXAMPLE_REGRESSION"
VALIDATOR_OK = "OK_B15_CACHED_EXAMPLE_OK"
KINDS_ALLOWED = ["positive_and_negative"]

VALIDATOR_NAME = "validate_b15_cached_example_integrity"
FIELD = "cached_example_observed"
...
def negative_bodies() -> dict[str, str]:
    failed = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    failed[0][FIELD] = "curated_example_failed"
    illegal = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    illegal[1][FIELD] = "cache_state_unknown"
    return {
        "negative_curated_example_failed": render_results(failed),
        "negative_illegal_cache_value": render_results(illegal),
    }
