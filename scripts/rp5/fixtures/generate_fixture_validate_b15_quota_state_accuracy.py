#!/usr/bin/env python3
"""
Fixture generator paired with validate_b15_quota_state_accuracy.
...
Negative cases exercised: two records whose quota_state_observed value
is outside the four canonical AssemblyAI quota states.

The generator performs no public network call and starts no public
exposure.

Emits OK_FIXTURE_VALIDATE_B15_QUOTA_STATE_ACCURACY on PASS or
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B15_QUOTA_STATE_ACCURACY"
SENTINEL_FAIL = "B15_QUOTA_STATE_MISREPRESENTED"
VALIDATOR_OK = "OK_B15_QUOTA_STATE_ACCURATE"
KINDS_ALLOWED = ["positive_and_negative"]

VALIDATOR_NAME = "validate_b15_quota_state_accuracy"
...
def negative_bodies() -> dict[str, str]:
    misrepresented = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    misrepresented[0][FIELD] = "AssemblyAI quota unknown"
    illegal = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    illegal[1][FIELD] = "quota_state_unconfirmed"
    return {
        "negative_misrepresented_quota_state": render_results(misrepresented),
        "negative_illegal_quota_value": render_results(illegal),
    }
