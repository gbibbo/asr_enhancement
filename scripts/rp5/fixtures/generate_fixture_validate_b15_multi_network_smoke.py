#!/usr/bin/env python3
"""
Fixture generator paired with validate_b15_multi_network_smoke.
...
Negative cases exercised:
  * a results file missing one required vantage point;
  * a results file whose record carries an incomplete
    coverage_item_outcomes mapping.

The generator performs no public network call and starts no public
exposure.

Emits OK_FIXTURE_VALIDATE_B15_MULTI_NETWORK_SMOKE on PASS or
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B15_MULTI_NETWORK_SMOKE"
SENTINEL_FAIL = "B15_MULTI_NETWORK_COVERAGE_GAP"
VALIDATOR_OK = "OK_B15_MULTI_NETWORK_SMOKE_OR_BLOCKED"
KINDS_ALLOWED = ["positive_and_negative"]

REPO_ROOT = pathlib.Path(__file__).resolve().parents[3]
VALIDATOR_PATH = REPO_ROOT / "scripts" / "rp5" / "validate_b15_multi_network_smoke.py"
...
def good_record(vp: str, coverage_items: list[str]) -> dict:
    return {
        "result_id": f"B15-SMOKE-RESULT-{vp}",
        "vantage_point": vp,
        "coverage_item_outcomes": {item: "COVERED" for item in coverage_items},
...
def render_results(records: list[dict]) -> str:
    parts = ["# B15 Multi-Network Smoke Results Fixture", ""]
    for rec in records:
...
def positive_body() -> str:
    return render_results(
        [good_record(vp, ALL_COVERAGE_ITEMS) for vp in ALL_VANTAGE_POINTS]
    )


def negative_bodies() -> dict[str, str]:
    missing_vp = render_results(
        [good_record(vp, ALL_COVERAGE_ITEMS) for vp in ALL_VANTAGE_POINTS[:-1]]
    )
    short_records = [good_record(vp, ALL_COVERAGE_ITEMS) for vp in ALL_VANTAGE_POINTS]
    short_records[0]["coverage_item_outcomes"] = {
        item: "COVERED" for item in ALL_COVERAGE_ITEMS[:-2]
    }
    incomplete_items = render_results(short_records)
    return {
        "negative_missing_vantage_point": missing_vp,
        "negative_incomplete_coverage_item_outcomes": incomplete_items,
    }
...
def main() -> int:
    ...
    fixtures_root = (
        manifest_path.parent / "generate_fixture_validate_b15_multi_network_smoke"
    )
