#!/usr/bin/env python3
"""
Fixture generator paired with validate_b15_smoke_result_evidence.
...
Negative cases exercised:
  * a result record with a null evidence_reference (fabricated result);
  * a result record whose har_reference does not cite the B15
    multi-network HAR.

The generator performs no public network call and starts no public
exposure.

Emits OK_FIXTURE_VALIDATE_B15_SMOKE_RESULT_EVIDENCE on PASS or
...
SENTINEL_PASS = "OK_FIXTURE_VALIDATE_B15_SMOKE_RESULT_EVIDENCE"
SENTINEL_FAIL = "B15_SMOKE_RESULT_FABRICATED"
VALIDATOR_OK = "OK_B15_SMOKE_RESULT_EVIDENCE_TRACED"
KINDS_ALLOWED = ["positive_and_negative"]

REPO_ROOT = pathlib.Path(__file__).resolve().parents[3]
VALIDATOR_PATH = (
    REPO_ROOT / "scripts" / "rp5" / "validate_b15_smoke_result_evidence.py"
)
...
def good_record(vp: str) -> dict:
    return {
        ...
        "validator": "validate_b15_smoke_result_evidence",
        "marker": "B15_SMOKE_RESULT_FABRICATED",
    }
...
def negative_bodies() -> dict[str, str]:
    null_evidence = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    null_evidence[0]["evidence_reference"] = None

    wrong_har = [good_record(vp) for vp in ALL_VANTAGE_POINTS]
    wrong_har[1]["har_reference"] = "HAR-UNRELATED-000"

    return {
        "negative_null_evidence_reference": render_results(null_evidence),
        "negative_wrong_har_reference": render_results(wrong_har),
    }
...
def main() -> int:
    ...
    fixtures_root = (
        manifest_path.parent / "generate_fixture_validate_b15_smoke_result_evidence"
    )
