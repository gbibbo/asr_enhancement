...
- Hypothesis C1 (header-strip; confirmed): the Funnel terminator may strip the Authorization header.
- Hypothesis C2 (port-forward; confirmed): the Funnel terminator may forward to a FastAPI port with PUBLIC_DEMO_EXPOSURE false.
- Hypothesis C3 (route-mapping; confirmed): the Funnel serve configuration may map a route bypassing the recruiter middleware.
- Hypothesis C4 (interface-binding; confirmed): FastAPI may be bound to a non-loopback interface allowing direct bypass.
- Hypothesis C5 (flag-drift; confirmed): the FastAPI process behind the Funnel may have been started with PUBLIC_DEMO_EXPOSURE unset.
